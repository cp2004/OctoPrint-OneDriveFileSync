...
export default function FileBrowser () {
    ...
    const fetchFiles = (itemId: string = "root") => {
        if (itemId === "root") {
            ...
            return OctoPrint.simpleApiCommand(PLUGIN_ID, "foldersById", { id: itemId })
        }
    }

    const {data, isFetching, error: folderQueryError, refetch: refetchFolders} = useQuery(
    ...
    const {data: configData, isFetching: configDataFetching, refetch: refetchConfig} = useQuery(
        ["accounts"],
        () => {
    ...
    const handleActivateFolder = (folder) => {
        OctoPrint.simpleApiCommand(PLUGIN_ID, "setFolder", { id: folder.id, path: folder.path }).done(
            () => refetchConfig()
        )
    }
        ...
        <>
            <h5>Sync Folder</h5>
            {
                configDataFetching
                    ...
                    : (
                        <span>Currently configured sync folder: {configData?.folder.path ? <code>{configData.folder.path}</code> : "None"}</span>
                    )
            }
            <div className={"row-fluid"}>
                ...
                {!active && (
                    <button className={"btn btn-primary"} onClick={() => setActive(true)}>
                        <i className={"fa-fw " + (isFetching ? "fas fa-spin fa-spinner" : "far fa-folder-open")}/> Change folder
                    </button>
                )}
                {active && (
                    <button className={"btn btn-primary"} onClick={() => refetchFolders()}>
                        <i className={"fa-fw fas fa-sync " + (isFetching ? "fa-spin" : "")}/> {hasError ? "Retry" : "Refresh"}
                    </button>
                )}
            </div>
            {hasError && (
